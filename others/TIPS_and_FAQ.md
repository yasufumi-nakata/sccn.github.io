...
スクロール、Epochの除去、部品除去)および手動で選択するか、または
...
epochs (そして実際に除去ボタンを押して下さい)、epochsはあります
...
**Answer:**でfirfiltプラグインをチェック [EEGLAB プラグインページ](/others/EEGLAB_Extensions.html).
...
### スペクトルパワーとコヒーレンスの被験者間解析?

被験者/患者を乗り越える良い方法はありますか?
特にパワーとコヒーレンスに関して分析
被験者/患者間の統計的比較?
EEGLABのデータ出力?


**Answer:** 被験者間比較の場合、tftopo()は強力です。
...
いくつかのコンポーネント。 例えば、 "paroxysmal" のアーティファクト (被験者のような)
録音中に頭を傷つける)は、数が多い
...
現在、ICAを使ってアーティファクトを修正しています。
過去に、ひとつひとつひとつのエポックを別々に検査しました。
これらをアーティファクトアクティビティで特定し、ICAを訓練
各試験を別々に識別し、アーティファクトコンポーネントを削除します。
...
1.  不適切(例えば、paroxysmal)部分を視覚的に除去して下さい
    連続データ。
2.  データを適切な短いデータエポックに分離します。
3.  ICAが独立したコンポーネントを導き出す。
4.  誘導された部品に対する除去を検査に基づいて行う
    自分の財産。
...
ICAコンポーネントアクティビティを使用して、データを削除し、悪意のある試験を解除します。
...
FFT(mutlitaper)で観察されるようにスーパーガウスのコンポーネント。
EEGLABチュートリアル(1st_readme.txt) 質問マークに気付いたかどうか
Windows用のbinica(古いもの)は安定していました。 それは不安定ですか?
...
Windows用のbinica()の改良版をコンパイルします。主に 'pca' オプションを関連した改善, 必要ない
使用するため。
...
同じ EEG とアーティファクト プロセスは、アクティブな ' である可能性があります。
エポック。
...
('times','freqs','ersp','itc') は、ERP を出力するようにします。
...
トムソンメソッド(通常はマルチタッパーとして知られています)も良いです。
最初にデータを直角ベースに投影し、FFTを実行します。

それらはすべての同じような結果(FFT、pewelch、multitaper)を返すべきです。
Thomson メソッドはノイズに敏感ではないが、また
より複雑な使用。 welchを使うこともできると思います
マルチタッパーの上の方法。 それはすべての好みの問題です。
...
2\) は pop_epoch() 関数の代わりに epoch() 関数を使用します。epoch() 関数は直接レイテンシーを処理します。 その後、取得します
...
たとえば「word.doc」やExcelにエクスポートするには?

...
- **でき事およびepochingの修正:**のbiosig2eeglabeventおよびPOP_biosigは要求される場合延長でき事コードのためのCodeDescの処理およびESFのアノテーションをEEG.eventにインポートするを含むEDF+の解読の論理を改善します。
...
- **新機能**: EEGLABカラーマップが変更されました。 [詳細はこちら](https://sccn.ucsd.edu/pipermail/eeglablist/2024/017887.html)を参照
...
- **バグ修正**: STUDYでの問題を修正 [ICAコンポーネントクラスタリング](https://github.com/sccn/eeglab/issues/767) また、以下のICLabelのバグにも関連しています。
...
- **Clean_rawdataプラグイン**: 同じ被験者の複数の実行で clean_rawdata によって拒否されたチャネルをヒューズする新しいオプション。
...
- **グループ分析**: 同じ被験者とセッションのデータセットを渡るチャネルの拒否を許可します。
...
- **テスト**:Githubを使用した新しいテストフレームワーク [アクション](https://github.com/sccn/eeglab_tests/actions)を参照してください。 コンパイルされた EEGLAB のバージョンの迅速なテストのために、sample_data フォルダに新しいバイナリテストファイルが追加されました。
...
- ** ASR**を用いたアーティファクト除去: Clean_rawdataプラグインにより、チャネルのサブセット処理が可能
...
- **Octave**:GUIサポートの改善
- **LIMO**:LIMOの互換性を改善し、被験者ごとの複数のセッションを使用する可能性を可能にしました
...
- **プラグインサポート**: 現在のMATLABパスの*plugins*サブフォルダにプラグインを置くことを可能にします
...
-   <b>階層イベント記述子(HED):</b> ツールの改善
    HED-2システムにおけるSTUDYレベルのデータセットイベントのアノテーション
    そしてHED-taggedのエポキシを抽出するため。
-   <b>ICLabel</b>: ICLabelのサポートと互換性の向上
    プラグイン。
-   <b>LIMO</b>: LIMOのサポートと互換性の向上
    プラグイン。
...
-   <b>STUDY処理機能における単一trial処理:</b>
    LIMO(LInear)と互換性のある新しいSTUDYフレームワークが含まれています
    EEGデータに適用されるMOdeling。
...
-   サポート <b>ろ過するときチャネルの選択</b> 追加された -
    ...
    タイプ。
-   <b>データインポートメニュー</b> クリーンアップされ、直接リンクを追加
    一般的なインポートプラグイン。

---
layout: default
parent: LIMO
grand_parent: Plugins
render_with_liquid: false

title: 09.2.-Between-Subjects-Categorical-Designs
nav_title: "09.2. 被験者間カテゴリ設計"
long_title: 09.2.-Between-Subjects-Categorical-Designs
---
ここ1ウェイANOVAは、親しみやすく、見慣れない、スクランブルな顔を再現しますが、2つの年齢グループでデータを分割します。 もちろん、我々は単にこれらのファイルを使用するLIMO MEEGで、それらを編集し、各グループのためのコピーを保存し、txtファイルを取ることができます。 ここでは、STUDY デザイン グループに追加する対象モデルを再入力します。これにより、グループごとに txt ファイルを保存します(ただし、被験者ごとに見積もりを変更しません)。 一部の被験者は未指定の年齢を持つため、メディアン(図35)に基づいて3つのグループを作成します。

グループ1は26の下にあります:サブ3、8、15、16、17、18
グループ 2 上記または等しい 26: サブ 2, 5, 9, 10, 11, 12, 14
グループ 3: サブ 4, 6, 7, 13, 19 未指定

![図35. 編集研究](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_meeg/master/resources/images/35.jpg)
_図35. STUDYグループを追加編集_

コマンドラインで pop_study typing を使って勉強を更新できます。
```matlab
cd(STUDY.filepath)
[STUDY ALLEEG] = std_editset( STUDY, ALLEEG, 'commands',{{'index',2,'group','1'}, ...
    {'index',7,'group','1'},{'index',14,'group','1'},{'index',15,'group','1'}, ...
    {'index',16,'group','1'},{'index',17,'group','1'},{'index',1,'group','2'}, ...
    {'index',4,'group','2'},{'index',8,'group','2'},{'index',9,'group','2'}, ...
    {'index',10,'group','2'},{'index',11,'group','2'},{'index',13,'group','2'}, ...
    {'index',3,'group','3'},{'index',5,'group','3'},{'index',6,'group','3'}, ...
    {'index',12,'group','3'}, {'index',18,'group','3'}}, 'updatedat','off','rmclust','on');
[STUDY, EEG] = pop_savestudy( STUDY, EEG, 'savemode','resave');
```

モデルを推定し、フェイスタイプで1stデザインを選択。 LIMO/Beta/conファイルのグループごとに分割して、テキストファイルを作成する。

ここからは2ndレベルの分析を実行できます。
- [繰り返し要因を持つ被験者のANOVA間](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_meeg/wiki/9.-Between-subjects%E2%80%99-ANOVAs-with-repeated-factors)
- [2つのサンプルtテスト](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_meeg/wiki/10.-Two-sample-t-tests)


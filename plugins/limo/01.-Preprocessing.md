...
このチュートリアルでは、使い方を教えることを目指しています [LIMO MEEG](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_tools/wiki)、との組合せで [EEGLABスタディ](https://sccn.ucsd.edu/wiki/Chapter_02:_STUDY_Creation)を参照 始めるためには、慣れている必要があります [EEGLAB](https://sccn.ucsd.edu/eeglab/index.php)を参照 また、MATLAB パスに LIMO ツールのサブディレクトリがリストされていることを確認してください。

このチュートリアルで使用されるデータは、 [ウェイクマンとヘンソン (2015)](https://www.nature.com/articles/sdata20151)を参照 この実験では、有名な、未知とスクランブルされた顔を見ながら、MEG-EEGデータを収集しました。 各画像は2回繰り返されます(例の50%と5〜10の刺激が他の50%に分けて)、その左右のインデックスフィンガーで2つのキーを押下し、各画像が平均比例した対称を示す。
...
データはに従って組織されます [脳イメージングデータ構造](https://bids.neuroimaging.io/), 特に [EEG BIDS拡張子](https://www.nature.com/articles/s41597-019-0104-8)を参照 ファイルの整理と名前付け方法を見て少し時間を費やす価値があります。 EEGLABはまた、専用の [bids-matlab-tools と呼ばれる BIDS ツール](https://raw.githubusercontent.com/sccn/bids-matlab-tools) BIDSデータセットの作成、エクスポート、インポート EEGLAB プラグインマネージャを使用して、下記のコードを実行する前にインストールする必要があります。

データをダウンロードしたら、以下のコードを実行できます。(特定のパスでXを変更する)ファイルをコピーして貼り付けて、絶え間ない処理済みデータを取得できます。 あるいはダウンロード [コードはこちら](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_meeg/master/resources/code/henson_wakeman_preprocess.m)を参照 患者様は、このステップは数時間かかります。
...
各デザインはここから作成できます。次のセクションでは、複数の異なるデザインを順に示します。まずは「要因のリスト」を見てみましょう([図5](https://raw.githubusercontent.com/LIMO-EEG-Toolbox/limo_meeg/master/resources/images/5.jpg))。このリストには、デザイン作成に使用できるすべてのカテゴリ変数が表示されます(独立変数のリスト要素を選択した場合に表示されます)。この要因リストは、LIMOが第1レベルで使用します。次のセクションでは、1要因ANOVAを使って顔タイプの効果を検定します(有名な顔、未知の顔、スクランブル顔の比較)。

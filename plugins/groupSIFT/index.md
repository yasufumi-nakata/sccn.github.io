...
[![YouTubeチュートリアルビデオのサムネイル](http://img.youtube.com/vi/jHngHEIsg7Q/0.jpg)](http://www.youtube.com/watch?v=jHngHEIsg7Q)
...
-   私はもともとフェドラ22でMATLAB R2013a runninigでそれを開発しました
    1600 x 1200の決断の二重モニターが付いている(64bit)。     プロジェクト、MATLAB 2017b、EEGLAB 14.1.2、およびSIFT 1.52 用に更新しました。
...
-   ICAで処理されるEEGLAB .setファイルが必要です。
    ...
    x 周波数 x 時間テンソル エンドの n 被験者! また、
    考慮したいデータポンツーパラメータ比。     また、DIPFITは
    ICLabel() プラグインを使用して、'brain' ラベルで IC を選択
    確率 \> 0.7, 例えば.
...
概要のプロットを点検し、outlierの被験者を取除くことを考慮して下さい
...
被験者(例えば、80%)は、非ゼロダイポール密度に貢献します。 プレス
'見積り用のコンピュートアバインド'ボタンで結果を確認します。
...
このプロットは、選択された双方向ダイポール密度、つまり重み付けされたグラフエッジを表示します。*Connectivity measure*では、ダイポール密度をrPDCまたはdDTFで重み付けします。
...
-   EEGLABメインウィンドウから、ツール -\> SIFT -\> 可視化 -\>
    BrainMovie3D. 私はこのために私のカスタマイズされた映画機能を使用します。
...
分析。 出力は、MATLAB の 'base' のワークスペースの 'dataSheet' として保存されます。
csvファイルとしてエクスポートできます。 A-Bの場合、
A と B を別々に実行します。(A と B は同じグラフエッジを持っています)。

# サブトラクションの場合には、個々の被験者データを出力する方法(02/26/2020更新)
...
同じメソッドは、内部被験者間のサブトラクションを取るために使用できます。
条件。 例えば、2つのネガティブなネガティブなデータを誤った場合
被験者のグループ、各グループのためのDeviant-Standardをサブトラクトしたい

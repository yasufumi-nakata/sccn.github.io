...
次にRELICAは、各分解で得られたICについて、ICスカルプマップの極性と生理学的妥当性の指標([Delorme ら, 2012](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0030135))を計算します。その後、時間経過どうしの相関に基づいて、分離されたICをクラスタリングします。クラスター数は計算されたIC数(通常は入力チャネル数)です。あるブートストラップデータセットが1つのクラスターに2つ以上のICを寄与した場合、そのブートストラップ分解は収束に失敗した可能性があるため、計算から除外されます。各クラスターは、クラスター測定空間で重心に最も近いICとして選ばれた代表ICで識別できます。ブートストラップICクラスターのコンパクトさの品質指標(QIc)は、各ICの信頼性を示す内在的な尺度を提供します。RELICAは結果をわかりやすく可視化する機能も返します。
...
1.***ローカル計算を下げる:*** RELICA が実行された同じ EEGLAB セッションでは、RELICA の結果は EEG 構造サブフィールド *EEG.etc に保存されます。 RELICA*

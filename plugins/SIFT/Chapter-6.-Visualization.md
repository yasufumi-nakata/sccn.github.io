...
Time-Frequency Gridを作成する手順は、グリッドレイアウトを設計することです。
異なるVARベースの対策の時間頻度画像をプロットすることができます
上部の三角形、下の三角形、またはグリッドの対角。
...
オプションで生成されるGUI `pop_vis_TimeFrequencyGrid()` ほぼすべて
...
IC 8(ロー1)と13(ロー3)で情報の流れの変化を展示します。
...
250-600 ms のまわりで流れ、デルタ バンド (2-3 Hz) に移ります。
...
*Preview BrainMovie**パネルのスクロールバーをクリックします。
...
ms)、中(40 ms)、および端(606 ms)のボタンプレスタスク。

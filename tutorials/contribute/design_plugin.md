...
"pophelp(*pop_newtimef*);" の定義 [入力gui.m](http://sccn.ucsd.edu/eeglab/locatefile.php?file=inputgui.m) コマンドライン関数
...
EEGLABの歴史とやりとり機能拡張
' ' ' '
'catch_strings' は文字列パラメータをキャッチします。
...
-   *catch_strings.new_and_hist* : LASTCOM 変数 従います (if)
    EEGLAB 空室検索 新しいデータセットウィンドウが開きます。

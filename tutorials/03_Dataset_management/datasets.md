...
イラストの目的でデータセットを変更する必要がありますので、フィルタリングのみ行います。 メニューから <span style="color: brown">Tools > Filter the data > Basic FIR filter</span> を選択し、*Lower edge*の周波数の編集ボックスに**1** Hzと入力して*Ok*を押します。

データセットは、EeGLAB が変更されました。 デフォルト設定は、 翻訳済みデータセットを上へ。 このサイトを編集する
説明 -- 将来の参照のための新しいデータセットの正確な性質を保存します。 プレスでこれを行う
*Ok*は、

![](/assets/images/pop_newset_new.png)
...
コールメニュー項目 <span style="color: brown">ファイル → 環境設定</span> データセットとファイルの好みは、以下に示す最初の3つの項目です。
...
![](/assets/images/pop_newset_overwrite.png)
